...
public class JumpState : BaseState
{
    ...
    public override void OnStateEnter()
    {
        ...
        //new code
        Vector3 platformVelocity = machine.GetMovableVelocity(); // get velocity of platform you're standing on
        Vector3 jumpVelocity = new Vector3(rb.linearVelocity.x, jump, rb.linearVelocity.z);
        jumpVelocity += platformVelocity; // inherit platform motion

        rb.linearVelocity = jumpVelocity;
        if (machine.GetMoveInput().magnitude <= 0)
        {
            machine.PlayInstantAnimation("Jump");
        }
        else { 
            machine.PlayInstantAnimation("RunningJump");
        }
    }
